...
<script src="${pageContext.request.contextPath}/resources/js/dataTable.min.js"></script>    
 <script>           
...
<table id="example" class="row-border hover" style="width:100%">
       ...
       <c:forEach items="${dataTablePackage}" var="dataTablePackage">
		 <tr>
		 	...
		 	<td><img src="https://ddragon.leagueoflegends.com/cdn/9.24.2/img/champion/${dataTablePackage.champIMG}" style="cursor:pointer; width:35px; height:35px;" onclick="championDetail('${dataTablePackage.champName}');"/></td>
		 	<td>${dataTablePackage.champName}</td>
		 	<td><div class="red" style="width:${(dataTablePackage.champWinPercent/75)*100}%; height:10px; background-color:#1f8ecd; border-color:#1f8ecd;"></div>${dataTablePackage.champWinPercent}%</td>
		 	<td><div class="red" style="width:${(dataTablePackage.banPercent/90)*100}%; height:10px; background-color: #ee5a52; border-color: #c6443e;"></div>${dataTablePackage.banPercent}%</td>
		 	<td><div class="red" style="width:${(dataTablePackage.pickPercent/50)*100}%; height:10px; background-color:#5FB4FC; border-color:#5FB4FC;"></div>${dataTablePackage.pickPercent}%</td>
		 	<td><div class="red" ></div>${dataTablePackage.champKDA}:1</td>
		 	<td><div class="red" style="width:${dataTablePackage.highMinionKill/2}%; height:10px; background-color:#D0D2A1; border-color:#D0D2A1;"></div>${dataTablePackage.highMinionKill}</td>
		 	<td><div class="red" style="width:${(dataTablePackage.highGold/13000)*100}%; height:10px; background-color:#DEE261; border-color:#DEE261;"></div>${dataTablePackage.highGold}</td>
